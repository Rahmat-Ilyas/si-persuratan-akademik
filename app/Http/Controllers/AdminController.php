...
class AdminController extends Controller
{
	...
	public function storeitem(Request $request)
	{
		...
		return redirect('admin/pfu/suratselesai')->with('msg', 'Surat berhail dibuat');
	}
	...
	{
		$surat = Surat::where('status', 'Sudah Selesai')->get();
		$mahasiswa = Mhswa::all();
		$item = Item::all();
		$paraf = Paraf::all();
	...
	public function cetak($page, $id)
	{
		...
		// return $pdf->download();
		return view('admin.pfu.surat.print' . $page, compact('surat', 'item', 'paraf', 'tandatgn', 'mahasiswa'));
	}

	public function datamahasiswa()
	{
		$mahasiswa = Mhswa::all();
		return view('admin.pfu.mahasiswa.datamahasiswa', compact('mahasiswa'));
	}

	public function tambahmahasiswa()
	{
		return view('admin.pfu.mahasiswa.tambahmahasiswa');
	}
	...
	public function storemahasiswa(Request $request)
	{
		...
		$mhs->foto = "default_img.jpg";
		$mhs->password = bcrypt($request->nim);
		$mhs->save();
		return redirect('admin/pfu/mahasiswa/datamahasiswa')->with('msg', 'Data mahasiswa berhasil ditambahkan');
	}

	public function editmahasiswa(Request $request, $id)
	{
		$mhs = Mhswa::where('id', $id)->first();
		if (isset($request->password)) {
			$mhs->password = bcrypt($request->jurusan);
			$mhs->save();
			return redirect('admin/pfu/mahasiswa/datamahasiswa')->with('msg', 'Password mahasiswa berhasil diubah');
		} else {
			$mhs->nama = $request->nama;
			$mhs->nim = $request->nim;
			$mhs->jurusan = $request->jurusan;
			$mhs->save();
			return redirect('admin/pfu/mahasiswa/datamahasiswa')->with('msg', 'Data mahasiswa berhasil diedit');
		}
	}
	...
	public function hapusmahasiswa($id)
	{
		$mhs = Mhswa::where('id', $id)->first();
		$mhs->delete();
		return redirect('admin/pfu/mahasiswa/datamahasiswa')->with('msg', 'Data mahasiswa berhasil dihapus');
	}
